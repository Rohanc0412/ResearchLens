...
test("login, session restore, and logout", async ({ page }) => {
  ...
  await page.route("http://localhost:8017/auth/login", async (route) => {
    await json(route, {
  ...
  await page.route("http://localhost:8017/auth/logout", async (route) => {
...
test("conversation flow, live run progress, and artifacts", async ({ page }) => {
  ...
  await page.route("http://localhost:8017/conversations/conversation-1", async (route) => {
    await json(route, {
      id: "conversation-1",
      tenant_id: "tenant-1",
  ...
  await page.route("http://localhost:8017/conversations/conversation-1/messages", async (route) => {
  ...
  await page.route("http://localhost:8017/conversations/conversation-1/runs", async (route) => {
    await json(route, {
      run: {
        id: "run-1",
        project_id: "project-1",
  ...
  await page.route("http://localhost:8017/runs/run-1", async (route) => {
    await json(route, {
      id: "run-1",
      project_id: "project-1",
      conversation_id: "conversation-1",
  ...
  await page.route("http://localhost:8017/runs/run-1/artifacts", async (route) => {
    await json(route, [
      {
        id: "artifact-1",
        run_id: "run-1",
  ...
  await page.route("http://localhost:8017/runs/run-1/evidence", async (route) => {
    await json(route, {
      run_id: "run-1",
      project_id: "project-1",
      conversation_id: "conversation-1",
  ...
  await page.route("http://localhost:8017/runs/run-1/evidence/sections/overview", async (route) => {
    await json(route, {
      section_id: "overview",
  ...
  await page.route("http://localhost:8017/runs/run-1/evaluation", async (route) => {
    await json(route, {
  ...
  await page.route("http://localhost:8017/runs/run-1/repair", async (route) => {
    await json(route, null);
  });
  await page.route("http://localhost:8017/artifacts/artifact-1/download", async (route) => {
    await route.fulfill({
      status: 200,
      headers: {
