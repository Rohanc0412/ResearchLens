...
(select count(*) from repair_results rr
 where rr.run_id = r.id and rr.changed is true) as repaired_section_count,
(select count(*) from repair_results rr
...
exists(select 1 from repair_results rr
 where rr.run_id = d.run_id and rr.section_id = d.section_id and rr.changed is true) as repaired,
...
SECTION_TRACE_SQL = """
select d.section_id, d.title, d.section_order,
coalesce(case when rr.changed is true then rr.revised_text end, d.section_text) as canonical_text,
coalesce(
 case when rr.changed is true then rr.revised_summary end,
 d.section_summary
) as canonical_summary,
case when rr.changed is true then true else false end as repaired, rr.id as repair_result_id,
