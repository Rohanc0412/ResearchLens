...
class RunDraftingStageUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        await self._repository.replace_section_preparation(run_id=run_id, briefs=briefs)
        await self._draft_sections(briefs, progress=progress)
        drafts = await self._repository.list_persisted_section_drafts(run_id=run_id)
        ...
        *,
        ...
        async with semaphore:
            if await self._cancellation_probe.cancel_requested(run_id=brief.section.run_id):
                raise RuntimeError("Drafting canceled before section generation.")
            if progress is not None:
